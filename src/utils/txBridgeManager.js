...
import fetchWithRetry from '../requests/fetchWithRetry';
...
export default {
  ...
  async getQuotes(
    ...
    const getQuote = await fetchWithRetry(
      `${baseUrl}/v0/transfer/quote${queryStrings}`,
      {},
      3
    );
  ...
  async checkAllowance({ bridge, fromAddress, fromChain, from }) {
    ...
    const getAllowance = await fetchWithRetry(
      `${baseUrl}/v0/transfer/allowance${queryStrings}`,
      {},
      3
    );

    const { allowance } = getAllowance;
    let allowanceFormatted = window.ethers.utils.formatUnits(
      allowance,
      decimals
    );
  ...
  async sendTransfer({
    ...
    try {
      ...
      const [requestErr, txHash] = await window.ethereum
        .request({
  ...
  async getTransferStatus({
    ...
    const getTransferStatusRequest = await fetch(
      `${baseUrl}/v0/transfer/status${queryStrings}`,
      {}
    );
  ...
  async approveToken({
    ...
    const getApprove = await fetchWithRetry(
      `${baseUrl}/v0/transfer/approve${queryStrings}`,
      {},
      3
    );
  ...
  async buildNewAllEstimates({
    ...
    const successfullEstimatesNew = routes.map(route => {
      ...
      const generatedTransactionId = 1;
      const txData = {
        ...
        estimate: {
          ...
          maxSlippage:
            route.route[0].maxSlippage ||
            7497 /*check out this value afterwards*/,
        },
  ...
  async getAllActiveTxs() {
    ...
    return [...cbridgeActiveTxsFormatted];
  },
