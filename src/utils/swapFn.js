...
window.SwapFn = {
  ...
  calculateEstimatedTransactionCost(
    ...
    switch (chainId) {
      ...
      case '137':
        return this.estimateGasWithPolygonAbi(
          contract,
          fromToken,
          toToken,
          amountBN,
      ...
      case '100':
        return this.estimateGasWithXdaiAbi(
          contract,
          fromToken,
          toToken,
          amountBN,
      ...
      case '1666600000':
        return this.estimateGasWithHarmonyAbi(
          contract,
          fromToken,
          toToken,
          amountBN,
      ...
      case '1313161554':
        return this.estimateGasWithAuroraAbi(
          contract,
          fromToken,
          toToken,
          amountBN,
  ...
  async calculatePriceImpact(fromToken, toToken, amount, originAmount) {
    ...
    const tokenPrice = await CoingeckoManager.getTokenPrice(
      assetPlatform,
      fromTokenAddress
    );
    ...
    if (tokenPrice) {
      ...
      const pmExpectReturn = await this.getExpectedReturn(
        fromToken,
        toToken,
        Utils.parseUnits(smallAmount, fromToken.decimals)
      );
      const pm = BN(pmExpectReturn.returnAmount.toString());
      const poExpectReturn = await this.getExpectedReturn(
        fromToken,
        toToken,
        amount
      );
  ...
  getApproveStatus(token, amountBN) {
    return this.getAllowance(token).then(allowanceBN => {
  ...
  approve(tokenContractAddress, amountBN) {
    ...
    if (['1inch', 'paraswap'].includes(pathRoute)) {
      return PathFinder.getApproveTx(
  ...
  */
  ...
  async getExpectedReturn(fromToken, toToken, amount, networkChainId) {
    ...
    if (PathFinder.SupportedChainIds.includes(`${chainId}`)) {
      const { destAmount, route, distribution } =
        (await PathFinder.getQuote(
          fromToken.symbol,
          toToken.symbol,
          amount,
          chainId
      ...
      if (destAmount) {
        const returnAmount = new BN(destAmount)
          .times(10 ** toToken.decimals)
          .toFixed(0);

        localStorage.setItem('route', route);
        const result = { returnAmount, route, distribution };
  ...
  async swap(fromToken, toToken, amountBN) {
    ...
    const pathRoute = localStorage.getItem('route');
    if (['1inch', 'paraswap'].includes(pathRoute)) {
      const originAmount = new BN(amountBN.toString()).dividedBy(
        10 ** fromToken.decimals
      );
      return PathFinder.getSwap(
        fromToken.symbol,
        toToken.symbol,
        originAmount,
        pathRoute,
    ...
    return this.calculateMinReturn(fromToken, toToken, amountBN).then(
      ({ minReturn, distribution, expectedAmount }) => {
        /*
        returns(
          uint256 returnAmount
        )
  ...
  swapWithOneSplitAbi(
    ...
    return contract
      ...
      .then(transaction =>
        this.returnSwapResult(transaction, fromToken, toToken, amountBN)
      );
  },
  ...
  estimateGasWithOneSplitAbi(
    ...
    return contract.estimateGas
      ...
      .then(gasUnitsEstimated =>
        this.returnEstimatedGasResult(gasUnitsEstimated)
      );
  },
